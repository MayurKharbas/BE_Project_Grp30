...
fn.deriveAdditionalFields <- function(inputSubsetDF){
  ...
  inputSubsetDF$activity_hour <- format(inputSubsetDF$activity_start_time,"%H")
  
  # derive total activity from sms in and out, call in and out and internet traffic activity 
  inputSubsetDF$total_activity <- rowSums(inputSubsetDF[, c(4,5,6,7)],na.rm=T)
  return(inputSubsetDF)
}
...
# function used to plot the activity_hour Vs total_activity
fn.findPeekActivityByHours <- function(inputSubsetDF){
  # Sum the total acitivity by hours
  totalActivityDF <- aggregate(total_activity ~ activity_hour,inputSubsetDF,FUN=sum)
  
  totalActivityPlot <- ggplot(data=totalActivityDF, aes(x=activity_hour, y=total_activity)) + geom_bar(stat="identity")+theme_bw()
...
fn.findOptimalCluster <- function(inputSubsetDF){
  ...
  cdrActivityDF <- subset(inputSubsetDF,select=c("activity_hour","total_activity"))
  ...
  opt_cluster <- ggplot(data=clusterFrame, aes(x=Cluster, y=withinss, group=1)) + geom_line(colour="red", size=1.5) + geom_point(colour="red", size=4, shape=21, fill="white")+theme_bw()
  print(opt_cluster)
  }
...
fn.findCDRKMeans <- function(cdrActivityDF){
  # subset the required columns
  cdrActivityDF <- subset(cdrActivityDF,select=c("activity_hour","total_activity"))
  ...
  cdrActivityDF$activity_hour <- as.factor(cdrActivityDF$activity_hour)
  
  # melt the dataframe
  cdrActivityDF.melt <- melt(cdrActivityDF)
  
  # heat map plot for the clustering results
  cdrActivityClusterPlot <- ggplot(cdrActivityDF.melt, aes(activity_hour,cluster)) + geom_tile(aes(fill = value))+ scale_fill_gradient(low = "#FBCAC5",high = "#D66889")+theme_bw()
...
fn.cdrClusterAnalysis <- function(){
  ...
  #newdf1 <-  ddply(newdf, .(square_id , activity_start_time), summarize, total_activity = sum(total_activity))
  #newdf1 <-  ddply(newdf, .(square_id , activity_date , activity_hour), summarize, total_activity = sum(total_activity))
  #aggregate(total_activity ~ square_id + activity_start_time, FUN = "sum", data = newdf)
  #write.csv(newdf1,file = "C:\\Users\\Sameer\\Documents\\processed_1.csv",row.names = FALSE)
  #write.csv(newdf1,file = "F:\\BE Project\\Code\\ProcessedDataset\\November\\sms-call-internet-mi-2013-11-07.csv",row.names = FALSE)
  ...
  #K-means
  fn.findOptimalCluster(cdrAcitivityInputDF)
